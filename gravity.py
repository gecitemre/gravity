...
PLANET_COLOR=255.*(128*numpy.ones(3,dtype=int)>numpy.array(BG_COLOR))
COLLISION_COLOR=numpy.array((255.,0.,0.))
...
class Planet:
    def __init__(self,coordinate,velocity=numpy.zeros(2),acceleration=numpy.zeros(2),color=PLANET_COLOR.copy(),mass=1,radius=10,ID=None):
        ...
        self.ID = ID
    def redraw(self):
        canvas.itemconfig(self.ID,fill=rgb_color(self.color.astype(int)),outline=rgb_color(self.color.astype(int)))
        canvas.coords(self.ID,tuple(numpy.hstack((self.coordinate-self.radius,self.coordinate+self.radius))/SCALE))
    def move(self):
        self.coordinate += (self.velocity + self.acceleration*DT*PLAY_SPEED/2)*DT*PLAY_SPEED
...
def interact(planet1,planet2):
    ...
    if real_distance < 0:
        #elastic collision
        vector_a = 2*numpy.matmul(planet1.velocity-planet2.velocity,vector12)*vector12/(distance_squared*(planet1.mass+planet2.mass))
    ...
    #gravitation
    force=(planet1.mass*planet2.mass/pow(distance,3))*vector12 #gravitational constant is ignored.
    planet1.acceleration+=force/planet1.mass
    planet2.acceleration-=force/planet2.mass
...
def interact_p(pressed_planet,planet):
    ...
    if distance:
        if real_distance < 0:
            #elastic collision
            vector_a = 2*numpy.matmul(pressed_planet.velocity-planet.velocity,vector12)*vector12/(distance_squared*(pressed_planet.mass+planet.mass))
        ...
        #gravitation
        force=(pressed_planet.mass*planet.mass/pow(distance,3))*vector12 #gravitational constant is ignored.
        pressed_planet.acceleration+=force/pressed_planet.mass
...
def ButtonPress1(event):
    ...
    if pressed_planet==None:
        ...
        pressed_planet=Planet(coordinate,radius=radius,mass=4/3*numpy.pi*DENSITY*radius**3,ID=canvas.create_oval(tuple(numpy.hstack((coordinate-radius,coordinate+radius))/SCALE),fill=rgb_color(PLANET_COLOR.astype(int))))
...
def ButtonRelease1(event):
    ...
    pressed_planet.velocity=SCALE/PLAY_SPEED*(numpy.array((event.x,event.y))-old_cursor_coordinate)/(time()*1000-before)
    pressed_planet=None
    ...
    def create_universe() -> list[Planet]:
        ...
        for _ in range(INITIAL_PLANETS):
            ...
            while flag:
                ...
                for planet in planets:
                    if planet.radius+radius>=numpy.linalg.norm(planet.coordinate-coordinate):
                        break
                flag=False
            planets.append(Planet(coordinate,numpy.random.uniform(high = INITIAL_SPEED_CONSTANT*SCALE, size = 2),mass=4/3*numpy.pi*DENSITY*radius**3,radius=radius,ID=canvas.create_oval(tuple(numpy.hstack((coordinate-radius,coordinate+radius))/SCALE),fill=rgb_color(PLANET_COLOR.astype(int)))))
        return planets
    def edge(planet):
        global coordinate_difference
        edge_matrix = numpy.vstack((planet.coordinate <= planet.radius, planet.coordinate >= 2*CENTER-planet.radius))
        if edge_matrix.any():
            planet.color=COLLISION_COLOR.copy()
            change = (edge_matrix * (numpy.vstack((numpy.array((planet.radius, planet.radius)), 2*CENTER-planet.radius)) - planet.coordinate)).sum(axis=0)
    ...
    def create_universe() -> list[Planet]:
        ...
        for _ in range(INITIAL_PLANETS):
            ...
            while flag:
                ...
                coordinate=r*(numpy.array(numpy.cos(theta),numpy.sin(theta))+CENTER)
                radius = numpy.random.uniform(RADIUS_RANGE[0],RADIUS_RANGE[1])*SCALE
                for planet in planets:
                    if planet.radius+radius<numpy.linalg.norm(planet.coordinate-coordinate):
                        break
